...
interface HookState {
  t: number;
  objects: Record<string, { visible: boolean; kind: string; position: Vec3; label: string }>;
...
async function waitForHookReady(page: Page) {
  await page.waitForFunction(() => window.__AGL_DIMENSIONS__?.ready === true, undefined, { timeout: 20_000 });
}
...
test.describe('T-004 — /dimensions route and lazy boundary', () => {
  ...
  test('desktop nav has a Dimensions entry that navigates to /dimensions', async ({ page }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await page.goto('/');
    await page.locator('nav a[href="/dimensions"]', { hasText: 'Dimensions' }).click();
    await page.waitForURL('**/dimensions');
    expect(page.url()).toContain('/dimensions');
  });

  test('mobile nav menu has a Dimensions entry that navigates to /dimensions', async ({ page }) => {
    ...
    await page.locator('a[href="/dimensions"]:visible', { hasText: 'Dimensions' }).click();
    await page.waitForURL('**/dimensions');
    expect(page.url()).toContain('/dimensions');
  });
  ...
  test('/products issues zero requests for the 3D vendor chunk', async ({ page }) => {
    const threeRequests: string[] = [];
    page.on('request', (req) => {
      if (/vendor-three|DimensionsStage/.test(req.url())) threeRequests.push(req.url());
    });
  ...
  test('with WebGL unavailable, a fallback renders and nothing throws', async ({ page }) => {
    ...
    await expect(page.getByTestId('dimensions-fallback')).toBeVisible({ timeout: 15_000 });
    await expect(page.getByTestId('dimensions-fallback')).toContainText('3D is unavailable');
    expect(pageErrors).toEqual([]);
  });

  test('under prefers-reduced-motion, motion-mode reads "reduced" and the frame count settles', async ({ page }) => {
    ...
    await expect(page.getByTestId('motion-mode')).toHaveText('reduced', { timeout: 15_000 });
    ...
    const framesAfterSettle = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getRenderStats()!.frame);
    await page.waitForTimeout(700);
    const framesLater = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getRenderStats()!.frame);
    expect(framesLater).toBe(framesAfterSettle);
  });

  test('Tab from the top of the page reaches interactive controls with non-empty accessible names', async ({ page }) => {
    await page.goto('/dimensions');
    await waitForHookReady(page);
    ...
    for (const name of names) {
      expect(name.length, `an interactive control on /dimensions had no accessible name`).toBeGreaterThan(0);
    }
  });

  for (const width of [640, 768, 1024]) {
    test(`renders with no horizontal scroll at ${width}px`, async ({ page }) => {
      await page.setViewportSize({ width, height: 1000 });
      await page.goto('/dimensions');
      await page.waitForLoadState('networkidle');
      await page.screenshot({ path: `../test-results/dimensions-${width}.png`, fullPage: true });
...
test.describe('T-005 — 3D spatial model and navigation', () => {
  ...
  test('the scene renders real geometry: triangles > 0 and draw calls > 0', async ({ page }) => {
    await page.goto('/dimensions');
    await waitForHookReady(page);
    const stats = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getRenderStats());
    expect(stats).not.toBeNull();
    expect(stats!.triangles).toBeGreaterThan(0);
    expect(stats!.calls).toBeGreaterThan(0);
  });

  test('a pointer drag across the canvas changes the camera position', async ({ page }) => {
    await page.goto('/dimensions');
    await waitForHookReady(page);
    ...
    const before = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getCamera().position);
    await page.mouse.move(box.x + box.width * 0.3, box.y + box.height * 0.5);
    await page.mouse.down();
    await page.mouse.move(box.x + box.width * 0.3 + 150, box.y + box.height * 0.5, { steps: 10 });
    await page.mouse.up();
    await page.waitForTimeout(200);
    const after = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getCamera().position);
  ...
  test('a wheel event changes the camera distance', async ({ page }) => {
    ...
    const before = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getCamera().distance);
    await page.mouse.wheel(0, -300);
    await page.waitForTimeout(200);
    const after = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getCamera().distance);

    expect(Math.abs(after - before)).toBeGreaterThan(0.05);
  });

  test('keyboard controls orbit and dolly the camera by the same kind of position delta', async ({ page }) => {
    ...
    const afterOrbit = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getCamera().position);
    expect(euclidean(before, afterOrbit)).toBeGreaterThan(0.05);

    const distanceBefore = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getCamera().distance);
    for (let i = 0; i < 5; i++) await page.keyboard.press('PageUp');
    const distanceAfter = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getCamera().distance);
    expect(Math.abs(distanceAfter - distanceBefore)).toBeGreaterThan(0.05);

    await page.keyboard.press('Home');
    const afterHome = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getCamera().position);
    expect(euclidean(afterHome, before)).toBeLessThan(0.01);
  });

  test('a named, sourced model asset is fetched at runtime and reaches "ready"', async ({ page }) => {
    const modelResponse = page.waitForResponse((res) => res.url().includes('/models/toycar.glb'));
    await page.goto('/dimensions');
    const res = await modelResponse;
    expect(res.status()).toBe(200);
    await expect(page.getByText('Loading the 3D model')).toHaveCount(0, { timeout: 20_000 });
    await waitForHookReady(page);
    const state = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getState());
    const toycarObjects = Object.values(state.objects).filter((o) => o.kind === 'toycar');
    expect(toycarObjects.length).toBeGreaterThan(0);
  });

  test('with the model request stubbed to a 500, a stated error renders and nothing throws', async ({ page }) => {
    const pageErrors: Error[] = [];
    page.on('pageerror', (err) => pageErrors.push(err));
    await page.route('**/models/toycar.glb', (route) => route.fulfill({ status: 500, body: 'stubbed failure' }));
    await page.goto('/dimensions');
    await expect(page.getByTestId('dimensions-model-error')).toBeVisible({ timeout: 20_000 });
    expect(pageErrors).toEqual([]);
  });
});
...
test.describe('T-006 — 4D time and process simulation', () => {
  test('setting the scrubber updates the hook\'s reported t to the same value', async ({ page }) => {
    await page.goto('/dimensions');
    await waitForHookReady(page);
    await setRangeValue(page, '#dimensions-scrubber', 3);
    await page.waitForTimeout(50);
    const t = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getState().t);
    expect(t).toBeCloseTo(3, 5);
  });

  test('different t values show different visible object sets, and returning restores the first set', async ({ page }) => {
    await page.goto('/dimensions');
    await waitForHookReady(page);
  ...
  test('t = 0.5 reached via a slider set and via 60 quantised keyboard steps is byte-identical', async ({ page }) => {
    await page.goto('/dimensions');
    await waitForHookReady(page);
    ...
    const viaDirectControl = await page.evaluate(() => {
      const { revision: _revision, ...rest } = window.__AGL_DIMENSIONS__!.getState();
      return rest;
    });
  ...
  test('play advances t over wall-clock time; pause freezes it', async ({ page }) => {
    await page.goto('/dimensions');
    await waitForHookReady(page);
    ...
    const whilePlaying = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getState().t);
    expect(whilePlaying).toBeGreaterThan(0);

    await page.getByRole('button', { name: 'Pause simulation' }).click();
    const atPause = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getState().t);
    await page.waitForTimeout(1000);
    const afterWait = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getState().t);
    expect(afterWait).toBe(atPause);
  });

  test('the lifecycle is named in the UI and rendered stage labels match the model data', async ({ page }) => {
    await page.goto('/dimensions');
    await waitForHookReady(page);

    const modelStages = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getState().stages.map((s) => s.label));
    await expect(page.getByRole('heading', { name: 'Product Delivery Pipeline' })).toBeVisible();

    const renderedLabels = await page.getByTestId('dimensions-stage-label').allTextContents();
    expect(renderedLabels).toEqual(modelStages);
  });

  test('under reduced motion, autoplay does not start and the timeline stays operable by direct control', async ({ page }) => {
    await page.emulateMedia({ reducedMotion: 'reduce' });
    await page.goto('/dimensions');
    await waitForHookReady(page);

    const initialT = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getState().t);
    expect(initialT).toBe(0);
    await page.waitForTimeout(1200);
    const tAfterWaiting = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getState().t);
    expect(tAfterWaiting).toBe(0); // no autoplay: t never moved on its own

    await setRangeValue(page, '#dimensions-scrubber', 2);
    await page.waitForTimeout(50);
    const tAfterDirectControl = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getState().t);
...
async function waitForPhysicsLoaded(page: Page) {
  await page.waitForFunction(() => window.__AGL_DIMENSIONS__!.getPhysics() !== null, undefined, { timeout: 15_000 });
}

test.describe('T-007 — 5D interaction: raycast selection and the parallel DOM control tree', () => {
  test('clicking a rendered object selects it via raycast, matching the object under the click point', async ({ page }) => {
    await page.goto('/dimensions');
    await waitForHookReady(page);
    const canvas = page.locator('canvas');
    await canvas.scrollIntoViewIfNeeded();
    ...
    const selection = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getState().selection);
    expect(selection).toBe('platform');
    await expect(page.getByTestId('dimensions-selection')).toContainText('Selected: Loading platform');

    // The parallel DOM control tree reflects the same selection state.
    const outlineButton = page.getByTestId('dimensions-outline-item').filter({ hasText: 'Loading platform' });
    await expect(outlineButton).toHaveAttribute('aria-pressed', 'true');
  });

  test('clicking empty space (no geometry under the click point) clears the selection', async ({ page }) => {
    await page.goto('/dimensions');
    await waitForHookReady(page);
    const canvas = page.locator('canvas');
    await canvas.scrollIntoViewIfNeeded();
    ...
    await page.waitForTimeout(100);
    expect(await page.evaluate(() => window.__AGL_DIMENSIONS__!.getState().selection)).toBe('platform');
    ...
    await expect(page.getByTestId('dimensions-selection')).toContainText('Nothing selected');
  });

  test('selecting from the scene outline writes the same selection state a raycast would', async ({ page }) => {
    await page.goto('/dimensions');
    await waitForHookReady(page);
    const button = page.getByTestId('dimensions-outline-item').filter({ hasText: 'Loading platform' });
    ...
    await button.click();
    await expect(button).toHaveAttribute('aria-pressed', 'false');
    expect(await page.evaluate(() => window.__AGL_DIMENSIONS__!.getState().selection)).toBeNull();
  });
});

test.describe('T-007 — 5D physics: a real cannon-es simulation', () => {
  test('the physics chunk is a separate lazy chunk, not requested until "Run simulation" is clicked', async ({ page }) => {
  ...
  test('gravity produces a monotonically decreasing y across at least 30 consecutive hook reads, and a bounce reverses the velocity sign', async ({
      ...
      () =>
        new Promise<{ y: number; vy: number }[]>((resolve) => {
          ...
          function frame() {
            ...
            if (snap) out.push({ y: snap.bodies.ball.position.y, vy: snap.bodies.ball.velocity.y });
    ...
    const bounced = samples.some((s, i) => i > 0 && samples[i - 1].vy < -0.05 && s.vy > 0.05);
    expect(bounced).toBe(true);
  });

  test('physics is not scripted: two different impulse magnitudes produce two different resting positions', async ({ page }) => {
    await page.goto('/dimensions');
    await waitForHookReady(page);
    ...
    async function restingXAfterImpulse(magnitude: number): Promise<number> {
      ...
      await setRangeValue(page, '[data-testid="physics-impulse-slider"]', magnitude);
      await page.getByTestId('physics-impulse-apply').click();
      await page.waitForTimeout(3_000); // let friction/damping settle it
      return page.evaluate(() => window.__AGL_DIMENSIONS__!.getPhysics()!.bodies.ball.position.x);
    }
  ...
  test('scenario controls change simulation inputs and the outcome changes measurably in the hook state (getState, not just getPhysics)', async ({
    ...
    async function restingXInState(magnitude: number): Promise<number> {
      ...
      return page.evaluate(() => window.__AGL_DIMENSIONS__!.getState().objects['phys:ball'].position.x);
    }
  ...
  test('Reset returns the ball to its starting state and re-enables the 4D timeline scrubber', async ({ page }) => {
    await page.goto('/dimensions');
    await waitForHookReady(page);
    ...
    await expect(page.getByTestId('dimensions-physics-timeline-note')).toContainText('runs forward only');

    await page.waitForTimeout(1_500); // let it fall away from its start position
    const midFallPosition = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getPhysics()!.bodies.ball.position);
    expect(midFallPosition.y).not.toBeCloseTo(authoredStart.y, 1);

    await page.getByTestId('physics-reset').click();
    const afterReset = await page.evaluate(() => window.__AGL_DIMENSIONS__!.getPhysics()!.bodies.ball.position);
    expect(afterReset).toEqual(authoredStart);
    await expect(scrubber).toBeEnabled();
    await expect(page.getByTestId('dimensions-physics-timeline-note')).toHaveCount(0);
  });
});

test.describe('T-007 — 5D live data: a real external endpoint over HTTPS', () => {
  test('fetches from a real external endpoint and renders a value parsed from the intercepted response body', async ({ page }) => {
    const responsePromise = page.waitForResponse((res) => res.url().includes('api.open-meteo.com'));
    await page.goto('/dimensions');
    const response = await responsePromise;
    expect(response.ok()).toBeTruthy();
    const body = await response.json();
    const expectedTemp = (body.current_weather.temperature as number).toFixed(1);

    await expect(page.getByTestId('live-data-value')).toContainText(`${expectedTemp}°F`, { timeout: 15_000 });
  });
  ...
  test('the upstream host is named in the UI, and the rendered value updates when a second, different response arrives', async ({
    ...
    await page.goto('/dimensions');
    await expect(page.getByTestId('live-data-source')).toContainText('api.open-meteo.com');
    await expect(page.getByTestId('live-data-value')).toBeVisible({ timeout: 15_000 });
    ...
    await page.route('**/v1/forecast**', (route) =>
      route.fulfill({
        ...
        body: JSON.stringify({
          current_weather: { temperature: 12.3, windspeed: 45.6, winddirection: 10, time: '2026-01-01T00:00' },
        }),
      }),
    );
    await page.getByTestId('live-data-refresh').click();
    await expect(page.getByTestId('live-data-value')).toContainText('12.3°F', { timeout: 10_000 });
  });

  test('with the endpoint stubbed to a 500, a stated error naming the source renders and no stale value is shown', async ({ page }) => {
    await page.route('**/v1/forecast**', (route) => route.fulfill({ status: 500, body: 'stubbed failure' }));
    await page.goto('/dimensions');
    await expect(page.getByTestId('live-data-error')).toContainText('api.open-meteo.com', { timeout: 15_000 });
  ...
  test('a value shown, then a failure, replaces it entirely — a stale value is never left displayed as current', async ({ page }) => {
    await page.goto('/dimensions');
    await expect(page.getByTestId('live-data-value')).toBeVisible({ timeout: 15_000 });
