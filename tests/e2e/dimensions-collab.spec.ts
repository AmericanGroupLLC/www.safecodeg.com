...
async function waitForHookReady(page: Page) {
  await page.waitForFunction(
    () =>
      (window as unknown as { __AGL_DIMENSIONS__?: { ready: boolean } })
        .__AGL_DIMENSIONS__?.ready === true,
    undefined,
    {
      timeout: 20_000,
    }
  ...
  return page.evaluate(() =>
    (
      window as unknown as {
        __AGL_DIMENSIONS__: { getTransport(): TransportSnapshot };
      }
    ).__AGL_DIMENSIONS__.getTransport()
  );
}
...
test.describe("T-010/T-011 — unconfigured build (genuinely, no test seam, no anon key exists)", () => {
  test("T-018 F-4: no sandbox notice and no join control is offered — there is nothing for the notice to honestly describe", async ({
    page,
  }) => {
    await page.goto("/dimensions");
    await waitForHookReady(page);
  ...
  test('T-018 F-4: sandbox-notice wording ("public sandbox", "anyone who joins can move") does not leak anywhere on the page in this state', async ({
    page,
  }) => {
    await page.goto("/dimensions");
    await waitForHookReady(page);
  ...
  test("T-018 F-4: remote session control states the honest unconfigured reason, not a pointer at a join control that does not exist", async ({
    page,
  }) => {
    await page.goto("/dimensions");
    await waitForHookReady(page);
    ...
    await expect(page.getByTestId("remote-control-status")).not.toContainText(
      /join the shared stage above/i
    );
    await expect(
      page.getByTestId("remote-control-role-controller")
    ).toHaveCount(0);
  });
  ...
  test("the test hook reports the real transport status honestly: unconfigured, zero actors, zero ops", async ({
    page,
  }) => {
    await page.goto("/dimensions");
    await waitForHookReady(page);
  ...
  test("screenshot: the 6D panels in their honest unconfigured state", async ({
    page,
  }) => {
    await page.goto("/dimensions");
    await waitForHookReady(page);
    ...
    await page.screenshot({
      path: "../test-results/t010-unconfigured.png",
      fullPage: false,
    });
  });
});
...
test.describe("T-016 S-9 — configured but not yet joined (the Join button IS present and clickable)", () => {
  test("the sandbox notice is already visible when the page loads — before the Join button is ever clicked, not after", async ({
    ...
    await expect(page.getByTestId("collab-sandbox-notice")).toContainText(
      /public sandbox/i
    );
    await expect(page.getByTestId("collab-sandbox-notice")).toContainText(
      /anyone who joins can move/i
    );
...
test.describe("T-010 — two-page synchronisation, presence, digital twin, disconnection (loopback proof — see file header)", () => {
      ...
      () =>
        (
          window as unknown as {
            __AGL_DIMENSIONS__: {
              getState(): {
                objects: Record<string, { position: { x: number } }>;
              };
            };
          }
        ).__AGL_DIMENSIONS__.getState().objects.platform.position.x
    );
  ...
  test("presence is real: two pages each report 2 participants; closing page B brings page A back to 1", async ({
    context,
  }) => {
    await withFakeTransport(context);
    const pageA = await context.newPage();
  ...
  test("digital twin: a third page, opened after the first two left, reads the position the first left it at", async ({
    context,
  }) => {
    await withFakeTransport(context);
    const pageA = await context.newPage();
      ...
      () =>
        (
          window as unknown as {
            __AGL_DIMENSIONS__: {
              getState(): {
                objects: Record<string, { position: { z: number } }>;
              };
            };
          }
        ).__AGL_DIMENSIONS__.getState().objects.platform.position.z
    );
...
test.describe("T-011 — remote session control (browser-session-as-remote-endpoint)", () => {
  ...
  test("a command sent by the controller is applied and reported back by the controlled session — genuinely, over the wire", async ({
    ...
    await expect(
      controller.getByTestId("collab-participant-count")
    ).toContainText("2");
    ...
    await controller.screenshot({
      path: "../test-results/t011-controller-confirmed.png",
    });
    await controlled.screenshot({
      path: "../test-results/t011-controlled-received.png",
    });
  ...
  test('with nobody set to receive, a sent command reports "no response" honestly rather than a fabricated success', async ({
    ...
    await expect(
      controller.getByTestId("remote-control-command-status")
    ).toContainText(/no report received/i, { timeout: 6_000 });
...
test.describe("T-010/T-011 — accessibility", () => {
  ...
  test("Tab order through the 6D section reaches every control in visual order, with no element skipped or trapped", async ({
    ...
    for (let i = 0; i < 25; i++) {
      ...
      const info = await page.evaluate(() => {
        ...
        return (
          ...
          `${el.tagName}[${el.getAttribute("aria-label") ?? el.getAttribute("name") ?? ""}]`
        );
      });
      ...
      if (
        info === "xr-enter-ar" ||
        info === "xr-enter-vr" ||
        /XRPanel|Enter AR|Enter VR/.test(info ?? "")
      )
        break;
    }
    ...
    if (!indices.every(idx => idx !== -1))
      console.log("TAB ORDER CAPTURED:", order);
...
test.describe("T-010/T-011 — breakpoints, no horizontal scroll", () => {
  for (const width of [640, 768, 1024]) {
    test(`/dimensions at ${width}px, joined with both panels expanded, has no horizontal page scroll`, async ({
      context,
    }) => {
      await withFakeTransport(context);
      const page = await context.newPage();
      ...
      await page.screenshot({
        path: `../test-results/t010-collab-${width}.png`,
        fullPage: true,
      });
