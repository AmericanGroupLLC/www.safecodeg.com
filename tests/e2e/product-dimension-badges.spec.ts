...
test.describe('T-014 — no fabricated dimensional capability badge', () => {
  test('the Products list page renders zero dimension badges across all cards', async ({ page }) => {
  ...
  test('the Products list page shows no "Capability:" filter row (derived from data — none exists today)', async ({ page }) => {
    await page.goto('/products');
  ...
  for (const [path, mustContain] of REAL_DATABASE_ENTRIES) {
    test(`real detail page ${path} renders no dimension badge`, async ({ page }) => {
      await page.goto(path);
      await expect(page.getByText(mustContain).first()).toBeVisible();
      await expect(page.locator('[data-testid="dimension-badge"]')).toHaveCount(0);
    });
  }
  ...
  for (const [path, mustContain] of FALLBACK_GENERATED_ENTRIES) {
    test(`generateFallbackProduct-synthesised page ${path} renders no dimension badge`, async ({ page }) => {
      await page.goto(path);
      await expect(page.getByText(mustContain).first()).toBeVisible();
      await expect(page.locator('[data-testid="dimension-badge"]')).toHaveCount(0);
    });
  ...
  test('a slug with no database entry at all still renders no dimension badge (generateFallbackProduct path)', async ({ page }) => {
...
test.describe('T-014 — breakpoints (regression: existing filters still work)', () => {
  for (const width of [640, 768, 1024]) {
    test(`Products page renders with no horizontal overflow at ${width}px`, async ({ page }) => {
  ...
  test('search and platform filters still narrow the product grid (regression)', async ({ page }) => {
    await page.goto('/products');
    const countText = async () => page.locator('text=/Showing \\d+ products/').first().textContent();
