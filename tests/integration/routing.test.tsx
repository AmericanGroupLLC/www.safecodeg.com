...
const ROUTES = [
  ...
  { path: '/products/:slug', label: 'Product Detail', component: 'ProductDetail' },
...
describe('Product slug resolution', () => {
  ...
  it('handles hyphenated slugs', () => {
    expect(resolveSlug('/products/apex-market-watch')).toBe('apex-market-watch');
  });
});
...
describe('Navigation integration', () => {
  const NavWrapper = () => (
    <nav>
      {ROUTES.filter(r => !r.path.includes(':') && !['/privacy-policy','/terms','/support'].includes(r.path)).map(r => (
...
describe('Page metadata', () => {
  const PAGE_META = {
    '/': { title: 'American Group LLC — Building the Future of Enterprise Tech', description: 'A California-based technology company delivering 77+ enterprise-grade products' },
    '/products': { title: 'Products — American Group LLC', description: 'Enterprise-grade mobile apps and platforms' },
    '/about': { title: 'About — American Group LLC', description: 'Our story, mission, and team' },
    '/careers': { title: 'Careers — American Group LLC', description: 'Join our team' },
    '/contact': { title: 'Contact — American Group LLC', description: 'Get in touch' },
  };
