...
import {
  ...
  visibleObjectIds,
  findActiveStage,
} from "@/dimensions/model/timeline";
...
describe("visibleObjectIds — genuine state-of-t, not an animation (T-006)", () => {
  ...
  it("every stage-scoped object is invisible outside its own stage window", () => {
    ...
    expect(visibleObjectIds(PRODUCT_PIPELINE, 1).has("crate" as never)).toBe(
      true
    );
    expect(visibleObjectIds(PRODUCT_PIPELINE, 9).has("crate" as never)).toBe(
      false
    );
  });
  ...
  it("objects with stage=null (the platform) are visible at every t", () => {
    for (const t of [0, 4, 8, 12, 16]) {
      expect(
        visibleObjectIds(PRODUCT_PIPELINE, t).has("platform" as never)
      ).toBe(true);
    }
  });
});
