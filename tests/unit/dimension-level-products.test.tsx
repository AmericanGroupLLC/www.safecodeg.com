...
describe('Current product data carries no dimensional capability claim (OQ-3)', () => {
  ...
  it('none of the 9 real ProductDetail entries has a dimensionLevel set', () => {
    const withLevel = Object.values(productDatabase).filter(p => p.dimensionLevel !== undefined);
    expect(withLevel).toEqual([]);
  });

  it('generateFallbackProduct — which synthesises 49 of 58 detail pages — never sets dimensionLevel, for any slug', () => {
    for (const slug of ['imeasure', 'roomcraft', 'audiosuite', 'some-unknown-product-xyz']) {
...
describe.each([
  ...
  it.each(DIMENSION_LEVELS)('renders the short label and a descriptive accessible name for level %s', (level) => {
    const meta = DIMENSION_LEVEL_META[level];
    render(<Badge level={level} />);
    const badge = screen.getByLabelText(`Dimensional capability: ${meta.label}`);
  ...
  it('meets WCAG AA text contrast (>= 4.5:1) against both page backgrounds', () => {
    ...
    const textRgb = parseRgb(colorMatch![1].startsWith('#') ? hexToRgbCss(colorMatch![1]) : colorMatch![1]);
