...
describe('clsx utility', () => {
  it('combines class strings', () => expect(clsx('foo', 'bar')).toBe('foo bar'));
  it('handles conditional classes', () => expect(clsx('base', { active: true, disabled: false })).toBe('base active'));
  it('handles undefined/null gracefully', () => expect(clsx('base', undefined, null, '')).toBe('base'));
  it('handles arrays', () => expect(clsx(['a', 'b'], 'c')).toBe('a b c'));
});

// ── tailwind-merge ────────────────────────────────────────────────────────────
describe('tailwind-merge utility', () => {
  it('merges conflicting tailwind classes (last wins)', () => expect(twMerge('text-red-500', 'text-blue-500')).toBe('text-blue-500'));
...
describe('Route generation', () => {
  ...
  it('generates correct product route', () => expect(generateProductRoute('cognicore')).toBe('/products/cognicore'));
  it('generates correct vertical filter route', () => expect(generateVerticalRoute('enterprise-ai')).toBe('/products?vertical=enterprise-ai'));
  it('handles slugs with numbers', () => expect(generateProductRoute('app365')).toBe('/products/app365'));
});
...
describe('Internship Positions', () => {
  const internships = [
    { title: 'AI / LLM Mobile App Intern', area: 'AI Mobile', type: 'Internship', duration: '3-6 months' },
    { title: 'LLM & Generative AI Research Intern', area: 'AI Research', type: 'Internship', duration: '3-6 months' },
    { title: 'Blockchain & Web3 Developer Intern', area: 'Blockchain', type: 'Internship', duration: '3-6 months' },
    { title: 'Blockchain Security & Audit Intern', area: 'Blockchain', type: 'Internship', duration: '3-6 months' },
  ];

  it('has exactly 4 internship positions', () => expect(internships).toHaveLength(4));
  it('has 2 AI-related internships', () => expect(internships.filter(i => i.area.includes('AI'))).toHaveLength(2));
  it('has 2 blockchain-related internships', () => expect(internships.filter(i => i.area === 'Blockchain')).toHaveLength(2));
  it('all are marked as Internship type', () => internships.forEach(i => expect(i.type).toBe('Internship')));
  it('all have 3-6 month duration', () => internships.forEach(i => expect(i.duration).toBe('3-6 months')));
});
...
describe('Contact Form Validation', () => {
  ...
  it('accepts valid email formats', () => {
    ['user@example.com', 'contact@safecodeg.com', 'info@american-group.llc'].forEach(e => expect(validateEmail(e)).toBe(true));
  });
  it('rejects invalid email formats', () => {
    ['notanemail', 'missing@', '@nodomain.com'].forEach(e => expect(validateEmail(e)).toBe(false));
  });
...
describe('Company Information', () => {
  ...
  it('has correct company name', () => expect(info.name).toBe('American Group LLC'));
  ...
  it('has 77+ products', () => expect(info.products).toBeGreaterThanOrEqual(77));
  it('has exactly 8 verticals', () => expect(info.verticals).toBe(8));
  it('has valid contact email', () => expect(info.email).toMatch(/^[^\s@]+@[^\s@]+\.[^\s@]+$/));
  it('has valid internship email', () => expect(info.internshipEmail).toBe('internships@safecodeg.com'));
  it('was founded in 2018 or earlier', () => expect(info.founded).toBeLessThanOrEqual(2018));
});
...
describe('Rich Theme Color Tokens', () => {
  ...
  it('obsidian is a valid hex color', () => expect(tokens.obsidian).toMatch(/^#[0-9A-Fa-f]{6}$/));
  it('violet is a valid hex color', () => expect(tokens.violet).toMatch(/^#[0-9A-Fa-f]{6}$/));
  it('gold is a valid hex color', () => expect(tokens.gold).toMatch(/^#[0-9A-Fa-f]{6}$/));
