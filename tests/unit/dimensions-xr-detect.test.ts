...
import { probeQuickLook, probeWebGL, probeXR, resolveXrUiState, xrStatusText } from '@/dimensions/xr/detect';

const originalGetContext = HTMLCanvasElement.prototype.getContext;

function mockWebGL(available: boolean) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (HTMLCanvasElement.prototype as any).getContext = available ? () => ({}) : () => null;
}
...
describe('probeXR — the eleven-state machine, nine of which this probe can produce', () => {
  ...
  it('immersive-ar only supported ⇒ "ar-only"', async () => {
    mockWebGL(true);
    mockNavigatorXr({
      isSessionSupported: vi.fn().mockImplementation((mode: string) => Promise.resolve(mode === 'immersive-ar')),
    });
    const result = await probeXR();
  ...
  it('immersive-vr only supported ⇒ "vr-only"', async () => {
    mockWebGL(true);
    mockNavigatorXr({
      isSessionSupported: vi.fn().mockImplementation((mode: string) => Promise.resolve(mode === 'immersive-vr')),
    });
    const result = await probeXR();
  ...
  it('a REJECTED isSessionSupported ⇒ "blocked-by-policy", never "no-device" — the rule the whole decision exists for', async () => {
    mockWebGL(true);
    mockNavigatorXr({
      isSessionSupported: vi.fn().mockRejectedValue(new DOMException('blocked', 'SecurityError')),
    });
    const result = await probeXR();
  ...
  it('never throws — every branch resolves to a named state', async () => {
    mockWebGL(true);
    mockNavigatorXr({ isSessionSupported: vi.fn().mockRejectedValue(new Error('boom')) });
...
describe('resolveXrUiState — the session-lifecycle layer over the probe', () => {
  ...
  it('idle/requesting pass the underlying capability state through unchanged', () => {
    ...
    expect(resolveXrUiState('blocked-by-policy', 'requesting')).toBe('blocked-by-policy');
  });
});
...
describe('xrStatusText — the honesty contract as a single testable assertion', () => {
  ...
  it.each(nonRunningStates)('state "%s" never contains an affirmative session string', (state) => {
    const text = xrStatusText(state, { reason: 'the user declined the prompt.', mode: 'immersive-ar' });
  ...
  it('session-rejected names the actual rejection reason, not a generic message', () => {
    const text = xrStatusText('session-rejected', { reason: 'permission denied by the user' });
  ...
  it('the four unavailable-family states each name a distinct, specific reason (never a shared generic string)', () => {
    const texts = new Set(
      (['no-webxr', 'no-device', 'blocked-by-policy'] as const).map((s) => xrStatusText(s)),
    );
