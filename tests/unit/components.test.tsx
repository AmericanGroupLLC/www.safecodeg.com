...
const MockNavigation = ({ currentPath = '/' }: { currentPath?: string }) => (
  <nav role="navigation" aria-label="Main navigation">
    <a href="/" aria-label="American Group LLC home">American Group LLC</a>
    <ul>
      <li><a href="/" aria-current={currentPath === '/' ? 'page' : undefined}>Home</a></li>
      <li><a href="/products" aria-current={currentPath === '/products' ? 'page' : undefined}>Products</a></li>
      <li><a href="/about" aria-current={currentPath === '/about' ? 'page' : undefined}>About</a></li>
      <li><a href="/careers" aria-current={currentPath === '/careers' ? 'page' : undefined}>Careers</a></li>
      <li><a href="/contact" aria-current={currentPath === '/contact' ? 'page' : undefined}>Contact</a></li>
    </ul>
...
// Mock StatCard
const MockStatCard = ({ value, suffix, label, sub }: { value: number; suffix: string; label: string; sub: string }) => (
...
describe('Navigation component', () => {
  ...
  it('home link points to root', () => {
    render(<MockNavigation />);
    const homeLink = screen.getByRole('link', { name: 'American Group LLC home' });
...
describe('StatCard component', () => {
  it('renders value with suffix', () => {
    render(<MockStatCard value={77} suffix="+" label="Products Shipped" sub="Across 8 verticals" />);
    expect(screen.getByTestId('stat-value').textContent).toBe('77+');
  });

  it('renders label', () => {
    render(<MockStatCard value={8} suffix="" label="Industry Verticals" sub="AI to CyberSecurity" />);
    expect(screen.getByTestId('stat-label').textContent).toBe('Industry Verticals');
  });

  it('renders sub-label', () => {
    render(<MockStatCard value={99} suffix=".9%" label="Uptime SLA" sub="Enterprise guarantee" />);
    expect(screen.getByTestId('stat-sub').textContent).toBe('Enterprise guarantee');
  });

  it('has accessible region role', () => {
    render(<MockStatCard value={6} suffix="+" label="Years of Excellence" sub="Est. 2018" />);
    expect(screen.getByRole('region', { name: 'Years of Excellence' })).toBeTruthy();
  });
});
