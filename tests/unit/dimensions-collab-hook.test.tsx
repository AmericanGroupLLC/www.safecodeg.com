...
import type { ActorId, CollaborationTransport, ObjectId, TransportStatus } from '@/dimensions/transport/types';
...
function useLoopbackFor(sharedStorage: Storage) {
  (window as unknown as { __AGL_DIMENSIONS_TEST_TRANSPORT__?: () => CollaborationTransport }).__AGL_DIMENSIONS_TEST_TRANSPORT__ =
...
let activeSessions: Array<{ hook: ReturnType<typeof renderHook<ReturnType<typeof useCollaboration>, unknown>> }> = [];
...
afterEach(async () => {
  ...
  activeSessions = [];
  delete (window as unknown as { __AGL_DIMENSIONS_TEST_TRANSPORT__?: unknown }).__AGL_DIMENSIONS_TEST_TRANSPORT__;
  vi.useRealTimers();
});
...
describe('useCollaboration — two-page synchronisation proof (T-010, adapted per §6.7/this task\'s brief)', () => {
  ...
  it('T-016 S-9: resetTarget genuinely restores the shared object, and the reset itself is synced to the other session', async () => {
    ...
    await waitFor(() => expect(b.store.getSnapshot().objects.platform.position.x).toBeCloseTo(baseline.x + 5, 5));
  ...
  it('digital twin: a third session, joining after both others left, reads the position the first session left it at', async () => {
    ...
    await waitFor(() => {
      expect(b.store.getSnapshot().objects.platform.position.x).toBeCloseTo(3.3, 5);
    });
    ...
    await waitFor(() => {
      expect(c.store.getSnapshot().objects.platform.position.x).toBeCloseTo(3.3, 5);
    });
  });
...
describe('useCollaboration — T-011 remote session control (browser-session-as-remote-endpoint)', () => {
  it('a controller\'s command is applied by the controlled session, which reports back — the controller\'s "confirmed" state is derived from that report, not assumed from the command it sent', async () => {
    ...
    await waitFor(() => expect(a.hook.result.current.lastCommand?.status).toBe('confirmed'));
    expect(a.hook.result.current.lastCommand?.confirmedByActorId).toBe(b.hook.result.current.selfActorId);

    await waitFor(() => expect(b.hook.result.current.lastReceivedCommand).not.toBeNull());
    expect(b.hook.result.current.lastReceivedCommand?.fromActorId).toBe(a.hook.result.current.selfActorId);
    ...
    expect(a.store.getSnapshot().objects.platform.rev.actorId).toBe(b.hook.result.current.selfActorId);
  });
...
describe('useCollaboration — disconnection is honest', () => {
  it('a status change away from "connected" clears the presented peer list rather than leaving it looking current', async () => {
    ...
    let presenceCb: ((actors: readonly import('@/dimensions/transport/types').ActorPresence[]) => void) | undefined;
    const fake: CollaborationTransport = {
      status: { kind: 'idle' },
      join: vi.fn(async () => {
        presenceCb?.([
          { actorId: 'a' as ActorId, displayName: 'Alice', colorHex: '#ffffff', joinedAt: 1, lastSeenAt: 1 },
          { actorId: 'b' as ActorId, displayName: 'Bob', colorHex: '#000000', joinedAt: 1, lastSeenAt: 1 },
        ]);
      }),
      leave: vi.fn(async () => {}),
      publish: vi.fn(async () => {}),
...
describe('useCollaboration — T-016 finding S-11: a fixed, non-leaking error string', () => {
  ...
  function fakeWithFailure(overrides: Partial<CollaborationTransport>): CollaborationTransport {
    return {
      status: { kind: 'idle' },
      join: vi.fn(async () => {}),
      leave: vi.fn(async () => {}),
  ...
  it('join() rejecting with a hostile error message never reaches lastErrorMessage', async () => {
    ...
    const fake = fakeWithFailure({ join: vi.fn(async () => { throw new Error(HOSTILE_DETAIL); }) });
    (window as unknown as { __AGL_DIMENSIONS_TEST_TRANSPORT__?: () => CollaborationTransport }).__AGL_DIMENSIONS_TEST_TRANSPORT__ = () => fake;
    ...
    expect(result.current.lastErrorMessage).not.toMatch(/relation|constraint|hint|dimensions_room_state/i);
    expect(result.current.lastErrorMessage).toBe('Could not join the shared stage. Please try again in a moment.');
    // The detail IS available to a developer via the console, just never rendered.
    expect(consoleSpy.mock.calls.some((call) => String(call[1]).includes(HOSTILE_DETAIL))).toBe(true);
  ...
  it('a loadSnapshot() rejection (after a successful join) never leaks its detail either', async () => {
    ...
    const fake = fakeWithFailure({
      join: vi.fn(async () => {}),
      loadSnapshot: vi.fn(async () => { throw new Error(HOSTILE_DETAIL); }),
    });
    (window as unknown as { __AGL_DIMENSIONS_TEST_TRANSPORT__?: () => CollaborationTransport }).__AGL_DIMENSIONS_TEST_TRANSPORT__ = () => fake;
    ...
    expect(result.current.lastErrorMessage).toBe("The shared stage's saved state could not be loaded.");
    expect(result.current.lastErrorMessage).not.toMatch(/relation|constraint|hint|dimensions_room_state/i);
    expect(consoleSpy.mock.calls.some((call) => String(call[1]).includes(HOSTILE_DETAIL))).toBe(true);
    consoleSpy.mockRestore();
  });
