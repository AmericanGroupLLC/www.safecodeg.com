  ...
  return (
    ...
    >
      ...
      {status.kind !== "unconfigured" && (
        ...
        >
          This is a public sandbox. There is no login and no per-user ownership
          — anyone who joins can move the shared object below or reset it.
          Nothing private is stored here.
        </p>
      )}
        ...
        >
          <div>
            <label
              ...
              className="block text-[10px] uppercase tracking-widest font-mono mb-1"
              style={{ color: "rgba(167,139,250,0.6)" }}
            >
              Your display name
            </label>
          ...
          >
            ...
            {isBusy ? "Joining…" : "Join the shared stage"}
          </button>
        </form>
      ) : (
        <div className="mb-4">
          <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
            ...
            >
              {actors.length}{" "}
              {actors.length === 1 ? "participant" : "participants"}
            </span>
            <button
              type="button"
              onClick={onLeave}
          ...
          <ul
            ...
            aria-label="Participants in the shared stage"
          >
            {actors.map(actor => {
              ...
              return (
                <li key={actor.actorId as string}>
                  <button
                    ...
                    onClick={() =>
                      setExpandedActorId(
                        expanded ? null : (actor.actorId as string)
                      )
                    }
                  ...
                  >
                    ...
                    {expanded
                      ? ` — joined ${new Date(actor.joinedAt).toLocaleTimeString()}`
                      : ""}
                  </button>
                </li>
              );
          ...
          <div className="mb-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button
                type="button"
                data-testid="collab-move-left"
                aria-label="Move the shared platform left"
                onClick={() => onMoveTarget({ x: -NUDGE_STEP })}
              ...
              <button
                type="button"
                data-testid="collab-move-right"
                aria-label="Move the shared platform right"
                onClick={() => onMoveTarget({ x: NUDGE_STEP })}
                className="p-2 rounded-lg"
                style={{
                  border: "1px solid rgba(124,58,237,0.25)",
              ...
              <button
                type="button"
                data-testid="collab-move-forward"
                aria-label="Move the shared platform forward"
                onClick={() => onMoveTarget({ z: -NUDGE_STEP })}
                className="p-2 rounded-lg"
                style={{
                  border: "1px solid rgba(124,58,237,0.25)",
                  color: "white",
              ...
              <button
                type="button"
                data-testid="collab-move-back"
                aria-label="Move the shared platform back"
                onClick={() => onMoveTarget({ z: NUDGE_STEP })}
                className="p-2 rounded-lg"
                style={{
                  border: "1px solid rgba(124,58,237,0.25)",
          ...
          <p
            data-testid="collab-twin-status"
            className="text-xs mt-3"
            style={{ color: "rgba(255,255,255,0.5)" }}
          >
            Digital twin:{" "}
            {lastSavedAt
              ? `last saved by this session at ${new Date(lastSavedAt).toLocaleTimeString()}`
              : "not yet saved this session"}
            . {opsApplied} change{opsApplied === 1 ? "" : "s"} applied,{" "}
            {opsRejected} rejected.
          </p>
        </div>
      )}
    </div>
