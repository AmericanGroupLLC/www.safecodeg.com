...
import {
  Mail, Phone, MessageCircle, Clock, ChevronDown, ChevronUp,
  Smartphone, Shield, Zap, BookOpen, Bug, CreditCard, Download,
  CheckCircle, ExternalLink, ArrowRight, HelpCircle,
} from "lucide-react";
...
const categories = [
  {
    icon: Smartphone, color: "oklch(0.52 0.22 270)", emoji: "📱",
    title: "App Installation & Setup",
    desc: "Help with downloading, installing, and first-time setup of any AGL app.",
    topics: ["App won't install or download", "Account creation issues", "Login and authentication problems", "App permissions setup", "Device compatibility questions"],
  },
  {
    icon: Bug, color: "oklch(0.55 0.18 30)", emoji: "🐛",
    title: "Bug Reports & Technical Issues",
    desc: "Report crashes, errors, or unexpected behavior in any of our apps.",
    topics: ["App crashes or freezes", "Features not working as expected", "Data sync issues", "Performance problems", "UI display errors"],
  },
  {
    icon: Shield, color: "oklch(0.72 0.14 165)", emoji: "🔐",
    title: "Privacy & Data",
    desc: "Questions about your data, privacy settings, and deletion requests.",
    topics: ["Data deletion requests", "Privacy settings help", "GDPR / CCPA rights", "Data export requests", "Account deletion"],
  },
  {
    icon: CreditCard, color: "oklch(0.78 0.18 75)", emoji: "💳",
    title: "Billing & Subscriptions",
    desc: "Help with in-app purchases, subscriptions, and refund requests.",
    topics: ["In-app purchase issues", "Subscription management", "Refund requests", "Restore purchases", "Payment method problems"],
  },
  {
    icon: Download, color: "oklch(0.65 0.16 310)", emoji: "⬇️",
    title: "Updates & Compatibility",
    desc: "Issues with app updates, OS compatibility, and version-specific problems.",
    topics: ["App update not available", "OS version compatibility", "Feature changes after update", "Rollback requests", "Beta testing access"],
  },
  {
    icon: BookOpen, color: "oklch(0.60 0.20 200)", emoji: "📖",
    title: "How-To & Guides",
    desc: "Step-by-step guidance on using features across all our applications.",
    topics: ["Feature walkthroughs", "Video tutorials", "User manuals", "Best practices", "Tips and tricks"],
  },
];
      ...
      >
        ...
        <div className="container relative z-10 text-center">
          ...
          <p className="text-lg text-slate-300 max-w-2xl mx-auto mb-10">
            Support for all <strong className="text-white">77 apps</strong> published by American Group LLC on the App Store and Google Play. We're here to help.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href={`mailto:${SUPPORT_EMAIL}`}
              ...
              style={{ background: "oklch(0.52 0.22 270)", boxShadow: "0 4px 20px oklch(0.52 0.22 270 / 0.4)" }}>
              <Mail className="h-4 w-4" />
              {SUPPORT_EMAIL}
            </a>
            <a href={`tel:${USA_PHONE.replace(/\s|\(|\)|-/g, "")}`}
              className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold transition-all duration-200 hover:scale-105"
              style={{ background: "oklch(1 0 0 / 0.1)", color: "white", border: "1px solid oklch(1 0 0 / 0.2)" }}>
      ...
      <section className="py-8 border-b border-slate-100">
        <div className="container">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { icon: Clock, label: "Email Response", value: "24–48 hrs", color: "oklch(0.52 0.22 270)" },
              { icon: Zap, label: "Urgent Issues", value: "< 12 hrs", color: "oklch(0.78 0.18 75)" },
              { icon: CheckCircle, label: "Apps Supported", value: "77 Apps", color: "oklch(0.72 0.14 165)" },
              { icon: MessageCircle, label: "Support Hours", value: "Mon–Fri", color: "oklch(0.65 0.16 310)" },
            ].map(({ icon: Icon, label, value, color }) => (
              <div key={label} className="flex items-center gap-3 p-4 rounded-2xl bg-slate-50">
      ...
      <section className="py-20">
        <div className="container">
          <div className="text-center mb-12 reveal">
            ...
            <p className="text-slate-500 max-w-xl mx-auto">Select a category below or email us directly — we support all 77 apps across iOS and Android.</p>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories.map((cat) => {
              ...
              return (
                <div key={cat.title}
                  className="reveal group p-6 rounded-2xl border border-slate-100 bg-white hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer"
                  onClick={() => window.location.href = `mailto:${SUPPORT_EMAIL}?subject=${encodeURIComponent(cat.title + " - Support Request")}`}
                >
        ...
        <div className="container">
          <div className="max-w-3xl mx-auto">
            <div className="text-center mb-12 reveal">
              ...
              <p className="text-slate-500">Quick answers to the most common support questions.</p>
            </div>
        ...
        <div className="container">
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-12 reveal">
              ...
              <p className="text-slate-500">Our team is available across both offices. Reach out and we'll get back to you promptly.</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6 reveal">
              ...
              <div className="p-8 rounded-3xl border border-slate-100 bg-white"
                ...
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <div className="font-bold text-slate-900">American Group LLC</div>
                    <div className="text-xs text-slate-500">Santa Clara, California, USA</div>
                  </div>
                </div>
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50">
                    ...
                    <span className="text-sm text-slate-500">Mon–Fri, 9 AM–5 PM PST</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-8 rounded-3xl border border-slate-100 bg-white"
                ...
                <div className="flex items-center gap-3 mb-6">
                  <span className="text-3xl">🇮🇳</span>
                  <div>
                    <div className="font-bold text-slate-900">SafeCodeX Research Center</div>
                    <div className="text-xs text-slate-500">Hyderabad, India</div>
                  </div>
                </div>
                <div className="space-y-3">
                  <a href={`mailto:${SUPPORT_EMAIL}`}
                    ...
                    <span className="text-sm text-slate-700 group-hover:text-amber-700">{SUPPORT_EMAIL}</span>
                  </a>
                  <a href={`tel:+917416866689`}
                    className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 hover:bg-amber-50 transition-colors group">
                    <Phone className="h-4 w-4 text-amber-500 shrink-0" />
                    <span className="text-sm text-slate-700 group-hover:text-amber-700">{INDIA_PHONE}</span>
                  </a>
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50">
                    <Clock className="h-4 w-4 text-slate-400 shrink-0" />
                    <span className="text-sm text-slate-500">Mon–Fri, 9 AM–6 PM IST</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Quick links */}
            <div className="mt-8 p-6 rounded-2xl bg-slate-50 reveal">
              <div className="text-sm font-semibold text-slate-700 mb-4">Related Pages</div>
              <div className="flex flex-wrap gap-3">
                {[
                  ...
                  <Link key={href} href={href}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-slate-200 text-sm text-slate-600 hover:text-indigo-600 hover:border-indigo-200 transition-all">
        ...
        <div className="container">
          <div className="max-w-3xl mx-auto p-6 rounded-2xl border border-indigo-100 bg-indigo-50 reveal">
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="font-bold text-indigo-900 mb-2">Official App Store URLs for Developers</div>
                <div className="space-y-1.5 text-sm">
                  {[
                    { label: "Support URL", url: "https://safecodeg.com/support" },
                    { label: "Privacy Policy URL", url: "https://safecodeg.com/privacy-policy" },
                    { label: "Terms of Service URL", url: "https://safecodeg.com/terms" },
                    { label: "Marketing Website", url: "https://safecodeg.com" },
                  ].map(({ label, url }) => (
                    <div key={url} className="flex items-center gap-3">
                      <span className="text-indigo-500 font-medium w-40 shrink-0">{label}:</span>
                      <code className="text-indigo-800 bg-white px-2 py-0.5 rounded text-xs font-mono border border-indigo-100">{url}</code>
                    </div>
                  ))}
                </div>
              </div>
